...
def import_from_string(val, setting_name):
    ...
    try:
        ...
        msg = "Could not import '%s' for plugin setting '%s'. %s: %s." % (val, setting_name, e.__class__.__name__, e)
...
class PluginSettings:
    ...
    def __init__(self,
        ...
        self._user_settings = self.__check_user_settings(getattr(settings, self._namespace, {}))
    ...
    def __getattr__(self, attr):
        if attr not in self.defaults:
            raise AttributeError("Invalid access - Plugin settings attribute '%s' has missing key: '%s'" %
